...
user@VSCode:~/natch_quickstart$ ll
total 7389756
drwxrwxr-x  3 user user       4096 янв 30 20:08 ./
drwxr-xr-x 25 user user       4096 янв 25 21:16 ../
-rw-rw-r--  1 user user   23764992 янв 30 19:40 lubuntu64.iso
...
и выполним его, после чего увидим знакомое нам графическое окно установки ubuntu. Выполним установку версии с минимально желаемым набором параметров

```bash

```
Удалим из скрипта запуска указание подключения cdrom - для дальнешей работы он нам не потребуется
...
Для выполнения подготовительного скрипта сборки для генерации make-файла потребуется установить ряд дополнительных зависимостей, в том числе для моей системы это выполняется командой:
