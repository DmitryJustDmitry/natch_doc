...
Установим требуемое системное ПО, в т.ч. QEMU (*Подсказка: данная инсталляция требуется не для запуска Natch, но для создания образов ВМ. Natch содержит в своём составе требуемую для работы версию QEMU*): 
```bash
sudo apt install  -y curl qemu-system gcc g++ //g++ ставлю просто так, по предварительной оценке он не потребуется для данного примера
```
...
после чего увидим знакомое нам графическое окно установки ubuntu. Выполним установку версии с минимально желаемым набором параметров (при установке был созда пользователь с уч. данными test:test). *Подсказка: чтобы вывести курсор мыши из открытого графического окна нажмите Ctrl+Alt+G*
