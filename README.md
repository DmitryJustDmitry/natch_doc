...
*(актуально на момент 01.06.2022)*
...
Комплект поставки Natch доступен в облачном хранилище в двух форматах:

- образ Ubuntu 20 для VirtualBox с предустановленным защищенным Natch, необходимым ПО (pip3, vim) и доступом к VPN-серверу, раздающему лицензии

[Natch v.1.3.1 (виртуальная машина в формате ova)](https://nextcloud.ispras.ru/index.php/s/learning)

- бинарный дистрибутив (для использования с персональным сетевым ключем в случае отсутствия доступа к серверу лицензий)

[Natch v.1.3.1 (бинарный дистрибутив + инструкция по подъему VPN)](https://nextcloud.ispras.ru/index.php/s/learning?path=%2Fnatch%2Bvpn)
...
##### Этап 2. Формирование скриптового и конфигурационного окружения
...
**Проброс портов гостевой ОС в хостовую ОС**
...
Нажмем `Ctrl (в моём случае левый) + Alt + G`, выйдем в монитор qemu и выполним команду генерации снэпшота (займёт около 1 минуты) - я назвал его `ready_to_go`:
